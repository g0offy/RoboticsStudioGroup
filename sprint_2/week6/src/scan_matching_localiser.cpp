...
private:
    ...
    void occupancyGridToImage(const nav_msgs::msg::OccupancyGrid::SharedPtr grid)
    {
        ...
        const int grid_spacing = 2; // Lower value to allow more flexibility in points alignment
                                ...
                                std::vector<cv::DMatch> &goodMatches)
    {
        ...
        cv::Ptr<cv::ORB> orb = cv::ORB::create(1500, 1.2f, 8, 31, 0, 2, cv::ORB::HARRIS_SCORE, 31, 64); // Increased keypoints
